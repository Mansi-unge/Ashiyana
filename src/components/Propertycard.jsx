...
const PropertyCard = ({ card }) => {
  if (!card) return null; // Handle undefined card gracefully
  ...
  return (
    <div 
      ...
      onClick={() => navigate(`/Properties/${card._id.$oid}`)} // Use card._id.$oid for navigation
    >
      <div className="w-full h-44 relative">
        <img
          src={card.image || "placeholder.jpg"} // Fallback to a placeholder
      ...
      <span className="text-2xl">
        {truncate(card.title || "Unnamed Property", {
          length: 15, // Adjust length as per your needs
          separator: " ",
          omission: "...",
        })}
      </span>
      <span className="text-sm w-full md:w-60">
        {truncate(card.description || "No details available.", {
          length: 80, // Maximum length of characters
          omission: "...",
        })}
      </span>
    </div>
